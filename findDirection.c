...
struct Graph *loadMapFromFile(const char *filename)
{
    ...
    while (fscanf(file, "%d %d %d %s", &src, &dest, &distance, directionStr) == 4)
    {
        ...
        {
            printf("\n\t\t\t\tInvalid input for the direction. Please enter 'right', 'left', 'straight', or 'back'.\n");
            continue;
        }
...
int main()
{
    ...
    printf("\n\t\t\t\t=========================================\n");
    printf("\t\t\t\tWelcome, lets find your way in any map!\n");
    printf("\t\t\t\t=========================================\n");
    ...
    if (!graph)
    {
        printf("\n\t\t\t\tFailed to load the map. Exiting.\n");
        return 1;
    }
    ...
    while (true)
    {
        ...
        {
            int src, dest;
            printf("\nEnter the source and destination nodes to find paths: ");
            if (scanf("%d %d", &src, &dest) != 2)
            {
                printf("\nInvalid input for source and destination nodes.\n");
                continue;
            }
        ...
        case 4:
        {
            printf("\nExiting the Route finder in 3,2,1...! \n\n\t\t\t\tTerminated\n");
            for (int i = 0; i < graph->V; i++)
            {
                struct Node *temp = graph->adjList[i];
                while (temp)
        ...
        {
            int src;
            printf("\nEnter the Vertex from which you have to find shortest path to other vertices : ");
            scanf("%d", &src);
            dijkstra(graph, src);
            break;
        }
